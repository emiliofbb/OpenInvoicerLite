...
function initElectronListeners() {
    ...
    window.api.dataInit((_, values) => {loadCustomer(values);});
    
}

async function loadCustomer(values) {
    ...
    const customer = await window.api.getCustomer(values.id);
    const magicalForm = document.getElementById("magical-form");
    setMagicalFormData(magicalForm, customer);
...
function initListeners() {
    ...
    deleteBtn.addEventListener("click", async (event) => {
        const result = await window.api.deleteCustomer(parseInt(idInput.value));
        if (!result) {
    ...
    saveBtn.addEventListener("click", async (event) => {
        ...
        const result = await window.api.saveCustomer(formData);
        if (result.error) {
            console.error(result.error);
        } else {
